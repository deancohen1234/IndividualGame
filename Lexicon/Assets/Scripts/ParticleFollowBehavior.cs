...
public class ParticleFollowBehavior : MonoBehaviour {
    ...
    void Update()
    {
        ...
        for (int i = 0; i < particles.GetUpperBound(0); i++)
        {
            ...
            float forceToAdd = (particles[i].startLifetime - particles[i].remainingLifetime) * (10 * Vector3.Distance(Target.position, particles[i].position - transform.position));

            //Debug.DrawRay (particles [i].position, (Target.position - particles [i].position).normalized * (ForceToAdd/10));

            particles[i].velocity = (Target.position - particles[i].position - transform.position).normalized * forceToAdd;
